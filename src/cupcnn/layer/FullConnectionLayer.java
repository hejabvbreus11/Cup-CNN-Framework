...
public class FullConnectionLayer extends Layer{
	...
	private double dropoutRatio;//神经元随机失活率
	int[] randArr;
	
	public FullConnectionLayer(Network network,BlobParams layerParams, double dropoutRatio){
	...
	public void prepare() {
		...
		randArr = new int[output.get3DSize()];
		for(int rand:randArr) {
			rand = (new Random().nextDouble()<dropoutRatio)?0:1;
		}
		...
		if(w==null && b==null){
			...
			//表明公有output.get3DSize()个神经元，每个神经元有一个偏执
	...
	public void forward() {
		...
		for(int n=0;n<input.getNumbers();n++){
			for(int os=0;os<output.get3DSize();os++){//有多少个输出，当前层就有多少个神经元
				...
				//调用dropout层
				if(dropoutRatio>0 && dropoutRatio<1) {
					outputData[n*output.get3DSize()+os] = outputData[n*output.get3DSize()+os]*randArr[os];
				}
			}
		}
